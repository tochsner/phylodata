...
from phylodata.samples.blast_utils import extract_taxon_ids, run_blast
from phylodata.samples.taxon_utils import look_up_taxon_classification
...
def add_nucleotide_metadata(samples: list[Sample]) -> list[Sample]:
    ...
    for idx, classification in zip(nucleotide_sequence_idx, classification):
        if classification:
            ...
            samples[idx].common_name = classification[
                0
            ].scientific_name  # Initially set common_name to scientific_name
