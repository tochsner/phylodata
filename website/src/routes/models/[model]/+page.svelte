...
<div class="m-8 flex items-start justify-center gap-8">
	{#if md}
		<div
			class="prose prose-h1:text-3xl prose-h2:text-xl max-w-[800px] flex-1 rounded-xl bg-white p-6 shadow-lg shadow-gray-400/5"
		>
			<h1>{model.name}</h1>
	...
	<div class="flex w-2/5 max-w-[350px] min-w-56 flex-col gap-8">
		...
		<div class="rounded-xl bg-white p-4 pb-1 shadow-lg shadow-gray-400/5">
			...
			{#if model.website}
				<a
					class="flex items-center gap-2 py-3 hover:opacity-60"
					href={model.website}
					target="_blank"
				>
					<svg
